...
class UnalignedDataLoader():
    def initialize(self, source, target, batch_size1, batch_size2):       
        ...
        self.dataset_t = dataset_target
        self.paired_data = PairedData(data_loader_s1, data_loader_s2, data_loader_t, float("inf"))
...
class PairedData(object):
    ...
    def __next__(self):
        ...
        if (self.stop_A and self.stop_t) or self.iter > self.max_dataset_size:
            self.stop_A = False
            self.stop_t = False
            raise StopIteration()
        else:
...
class UnalignedDataLoader():
    def initialize(self, source, target, batch_size1, batch_size2):       
        ...
        self.dataset_t = dataset_target
        self.paired_data = PairedData(data_loader_s1, data_loader_t, float("inf"))
